...
int getDist(const Point pos, const Point target) {
    int sx = pos.x;
    int sy = pos.y;
...
int solve(Point left, Point right, int idx) {
    if (idx >= input.size()) return 0;// 인덱스를 다 돌았으면 종료

    // left와 right의 좌표가 동일하면 안됨
    if (left.x == right.x && left.y == right.y) return 999999;
    ...
    int l_weight = getDist(left, input[idx]);
    int r_weight = getDist(right, input[idx]);

    // 해당 위치로 갔을 경우 최소 비용의 합
    int v1 = l_weight + solve(input[idx], right, idx + 1);
    int v2 = r_weight + solve(left, input[idx], idx + 1);
